            ...
            for (int j = 0; j < 32; j++)
            {
                ...
                for (int k = 0; k < 32; k++)
                {
                    ...
                    if (y < h)
                    {
                        if (y < 2)
                        {
                            chunk.chunkData[idx] = GetID(Random.Range(219, 222), Random.Range(204, 206), Random.Range(104, 151), 255);
                        }
                        else
                        {
                            chunk.chunkData[idx] = GetID(Random.Range(126, 187), Random.Range(204, 217), Random.Range(90, 145), 255);
                        }
                    }
                    else
                    {
                        chunk.chunkData[idx] = 0;
                    }
                }
            }
